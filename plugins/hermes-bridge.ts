...
export function apply(ctx: any): void {
  ...
  function findSessionId(name: string): Promise<string | null> {
    return new Promise((resolve) => {
      execFile(
        (process.env.HERMES_BIN || 'hermes'), ['sessions', 'list', '--limit', '500'],
  ...
  function runHermes(args: string[], timeoutMs: number): Promise<string> {
    return new Promise((resolve, reject) => {
      execFile(
        (process.env.HERMES_BIN || 'hermes'), args,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute({ task, session, reset_session }: { task: string, session?: string, reset_session?: boolean }) {
      const startedAt = Date.now()
      const taskBrief = String(task).slice(0, 120)
      const timeoutMs = Number(process.env.HERMES_TIMEOUT) || 600000
      const args: string[] = ['chat', '-q', String(task)]
      if (session) {
        if (reset_session) {
          // 重置：先按标题找到旧会话 ID 删除，再新建同名会话
          try {
            const sid = await findSessionId(String(session))
            if (sid) await runHermes(['sessions', 'delete', sid, '--yes'], 30000)

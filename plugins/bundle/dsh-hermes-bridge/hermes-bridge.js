...
export function apply(ctx) {
    ctx.tools.register(defineTool({
        ...
        description: '调用本机 Hermes Agent（另一 AI agent）执行任务，实现双 agent 协作。'
            ...
            + '注意：调用可能耗时（30 秒~10 分钟），超时则返回已输出部分；结果最多返回 8000 字符。',
        parameters: {
            task: {
                type: 'string',
                required: true,
        ...
        async execute({ task }) {
            try {
                const out = await new Promise((resolve, reject) => {
                    execFile((process.env.HERMES_BIN || 'hermes'), ['chat', '-q', String(task)], { timeout: 600000, maxBuffer: 16 * 1024 * 1024, env: { ...process.env } }, (err, stdout, stderr) => (err ? reject(new Error(String(err.message) + ' | ' + String(stderr).slice(0, 300))) : resolve(stdout)));
                });
                const trimmed = (out || '').trim();
                return trimmed.slice(-8000) || '(空输出)';
            }
            catch (e) {

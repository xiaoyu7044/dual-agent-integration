    ...
    ctx.tools.register(defineTool({
        ...
        async execute({ task }) {
            ...
            const run = queue.then(() => new Promise((resolve, reject) => {
                execFile((process.env.HERMES_BIN || 'hermes'), ['chat', '-q', String(task)], { timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024, env: { ...process.env } }, (err, stdout, stderr) => (err ? reject(new Error(String(err.message) + ' | ' + String(stderr).slice(0, 300))) : resolve(stdout)));
            }));
            queue = run.catch(() => { });
            try {

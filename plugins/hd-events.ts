...
export function apply(ctx: any): void {
  ...
  let lastActive = 0
  try { lastActive = Math.max(statSync(CACHE).mtimeMs, Date.now() - STALL_MIN * 60000) } catch { /* 文件不存在：从 0 开始 */ }
  ...
  try {
    ...
    // 清理：进程退出时释放锁
    const cleanup = () => { try { unlinkSync(LOCK) } catch { /* ignore */ } process.exit(0) }

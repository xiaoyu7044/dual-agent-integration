...
const acquired = (() => {
  if ((globalThis as any).__hdEventsActive) return false
  try {
    if (existsSync(LOCK)) {
      const pid = Number(readFileSync(LOCK, 'utf8'))
      if (pid > 0) {
        try { process.kill(pid, 0); return false } catch { /* 旧实例已死，可接管 */ }
      }
    }
    writeFileSync(LOCK, String(process.pid))
    ;(globalThis as any).__hdEventsActive = true
    return true
  } catch { return true } // 锁失败不阻塞（保守：继续运行）
})()
...
const SUB = (() => {
  try {
    ...
    const sub = arr.find((s: any) => s.name === 'hd-events')
    if (sub) return { url: sub.url || sub.webhook_url || '', secret: sub.secret || '' }
  } catch { /* ignore */ }
...
function scanLatestMtime(dir: string, depth = 0): number {
  if (depth > 4) return 0
  let latest = 0
  try {
...
export function apply(ctx: any): void {
  if (!acquired || (globalThis as any).__hdEventsApplied) {
    console.log('[hd-events] 已有实例在运行（单实例锁/幂等保护），跳过')
    return
  }
  ;(globalThis as any).__hdEventsApplied = true
  ...
  const post = (url: string, body: string, sig: string, event: string, tries: number): Promise<void> =>
    fetch(url, {
      method: 'POST',
      headers: {
      ...
      if (!r.ok && tries > 0) return post(url, body, sig, event, tries - 1)
    }).catch(() => (tries > 0 ? post(url, body, sig, event, tries - 1) : undefined))

  const notify = (event: string, detail: string) => {
  ...
  try {
    ...
    const scanTimer = setInterval(() => {
      ...
      if (l > lastActive) markActive('scan')
    }, 30000)
    ...
    const cleanup = () => {
      try {
        unlinkSync(LOCK)
        ;(globalThis as any).__hdEventsActive = false
      } catch { /* ignore */ }
      clearInterval(timer)
